...
public class A_Filtering {
	...
	public void method2() {
		...
		names.stream()
			// filter는 중간처리 종류 중 하나, forEach는 최종처리 종류 중 하나!
			// 같은 중간 처리 끼리는 순서 상관 없으나 중간 다음엔 최종이여야 하며 최종 처리는 하나만 사용! 
		...
		students.stream()
				...
				// .filter((Student student) -> {return student.get~ && ~>=60 ;});
				.distinct()
				.forEach(student -> System.out.println(student));

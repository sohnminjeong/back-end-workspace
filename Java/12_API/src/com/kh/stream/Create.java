...
public class Create {
	...
	// 숫자 범위로 스트림을 생성 - IntStream, LonnStream, DoubleStream 등 사용 
	public void method1() {
		// 첫 번째 매개 값 ~ 두 번째 매개 값 이전까지의 값을 요소로 가지는 스트림 객체 생성 
		IntStream stream = IntStream.range(1, 10);  // 1부터 9까지의 합 
		int sum = stream.sum();
		System.out.println("range의 sum : " + sum);   // 45
		
		// 첫 번째 매개 값 ~ 두 번째 매개 값까지의 값을 요소로 가지는 스트림 객체 생성 
		stream = IntStream.rangeClosed(1, 10);  // 1부터 10까지의 합 
		sum = stream.sum();

...
public class Car {
	 ...
	 * - 클래스, 변수, 메서드, 생성자에도 상요되어 외부로부터 접근을 제한 
	 * 
	 * public : 접근 제한이 전혀 없음
	 * protected : 같은 패키지 내에서, 그리고 다른 패키지의 자손 클래스에서 접근 가능
	 * default(접근제어자를 적지 않은 경우) : 같은 패키지 내에서만 접근이 가능
	...
	/*
	 * 생성자 
	 * - 객체가 생성될 때 마다 호출되는 '인스턴스 초기화 메서드'
	 ...
	 * - 매개 변수가 없는 생성자 
	 * - 클래스에 생성자가 하나도 없으면 컴파일러가 기본 생서자를 추가 
	 * - 생성자가 하나라도 있으면 컴파일러는 기본 생성자를 추가하지 않음    (기본 생성자 : public Car(){})
	 *   
	 * */
	// 생성자 오버로딩! 
	public Car() {}
		// 생성자의 경우 접근제어자 : public 
		// Car : 메서드명, () : 호출
	...
	// 오버라이딩 : 부모 클래스의 메서드를 자식 클래스에서 다시 재정의 
		// 기본 반환은 주소값 반환이 맞으나 그걸 return 문장으로 변경하여 출력 
		//public String toString() {
			//return "색상 : " + color + ", 변속기 : " + gearType + ", 문의 개수 : "+ door;
		//	}
	
	@Override    // 마우스오른쪽 -> source -> GeneratetoString 클릭 시 자동으로 Override 생성 
	public String toString() {

/*
 *  패키지(package)
 *  - 홀더를 만들어서 파일을 저장하고 관리하듯이 패키지를 만들어 클래스를 저장하고 관리한다. 
 *  
 *  - 패키지는 클래스를 유일하게 만들어주는 식별자 역할을 한다.
 *  
 *  - 모든 클래스는 하나의 패키지에 속하며, 패키지가 선언되지 않는 클래스는 자동적으로 이름 없응 패키지(default)에 속하게 된다.
 * */
package com.kh;
/*
 * 클래스(class)
 * - 자바에서 모든 코드는 반드시 클래스 안에 존재해야 하며 서로 관련된 코드들은 그룹으로 나누어 별도의 클래스를 구성한다.
 * 
...
/*
 * 네이밍 규칙
 * - 패키지명 : 소문자로 작성, 회사들 간에 패지키가 서로 중복되지 않도록 회사의 도메인 이름으로 패키지를 만든다.
 * 			

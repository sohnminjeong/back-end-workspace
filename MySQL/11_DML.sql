...
INSERT INTO emp(emp_id, dept_title) VALUES(700, '마케팅팀');
...
--  dept_title, hire_date는 기본값이 있어서 지우기 가능 
INSERT INTO emp(emp_id, emp_name)
SELECT emp_id, emp_name      
FROM kh.employee
 LEFT JOIN kh.department ON(dept_code = dept_id);
-- dept_code 추가 : 컬럼 추가 -> ALTER TABLE 테이블명 ADD 컬럼명 자료형;
...
FROM kh.employee
WHERE 1=0;   -- WHERE 1 = 0;  : 값이 아닌 틀만 복사 
...
/*
    ...
    - 별도의 조건을 제시할 수 없고(WHERE절x) ROLLBACK이 불가능하다.
		(DELETE절은 ROLLBACK 가능)
...
ROLLBACK; -- DELETE FROM은 ROLLBACK 가능!

TRUNCATE TABLE dept_copy;
TRUNCATE TABLE emp_salary;

ROLLBACK; -- TRUNCATE TABLE은 ROLLBACK 불가능!

DROP TABLE dept_copy;
DROP TABLE emp_salary;

ROLLBACK; -- DROP TABLE도 ROLLBACK 불가능!
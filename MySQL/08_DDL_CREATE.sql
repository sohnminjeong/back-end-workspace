...
/*
    ...
    * 자료형
    1. 문자 
		- CHAR / **VACHAR** : 고정 및 가변 길이 문자, 반드시 크기 지정
	...
	3. 날짜 및 시간
		- **DATE** : 날짜 저장하는데 사용
        - TIME : 시간 저장하는데 사용
        - DATETIME / TIMESTAMPE : 날짜와 시간을 함께 저장 
	4. 불리언
...
CREATE TABLE member(
	mem_no INT, mem_id VARCHAR(20), eme_pwd VARCHAR(20),
    mem_name VARCHAR(20),
    gender CHAR(3),
    phone VARCHAR(13),
    email VARCHAR(50),
...
CREATE TABLE mem_check(
    ...
    email VARCHAR(50),
    mem_date DATE, 
    CONSTRAINT mem_gender_check  CHECK (gender IN('남', '여')),  -- 제약 조건명 걸기
...
-- > check 제약조건 뿐만 아니라 null도 처리해야 된다는 것! (CHECK 조건이 아닌 NOT NULL 조건이 필요)
...
-- > 기본키에 null을 담으려고 해서 문제 발생! (not null 제약조건 위비)
...
DELETE FROM mem_grade
WHERE grade_code = 30;
-- > 자식테이블(member)에서 30이라는 값을 사용하지 않기 때문에 삭제 O
...
DELETE FROM mem_grade
WHERE grade_code = 10;
-- > 잘 삭제됨! 단, 10을 가저다 쓰고 있던 자식 데이터 값은 NULL로 변경 
...
DELETE FROM mem_grade
WHERE grade_code = 10;
-- >> ON DELETE SET NULL(grade_code 10제외해서 null로 표시)과 다르게 아예 grade_code가 10인 정보행이 전부 삭제됨 
...
DROP TABLE member;
CREATE TABLE member(
	mem_no INT AUTO_INCREMENT PRIMARY KEY,  -- 컬럼 레벨 방식 
    -- AUTO_INCREMENT : mem_no를 지정하지않아도 자동 순번으로 지정됨(MySQL에만 있는 기능)
    mem_name VARCHAR(20) NOT NULL,
    mem_age INT,
    hobby VARCHAR(20) DEFAULT '노래',
    -- enroll_date DATETIME DEFAULT NOW()
    enroll_date DATE DEFAULT (current_date())  -- CURRENT_DATE 직접 사용 불가(괄호안에표기) 
);
...
-- 내용은 복사되지 않고 데이터 테이블(틀)만 복사됨
-- 모든 행에 대해서 매번 false이기 때문에 테이블의 구조만 복사되고 데이터 값은 복사되지 X

DROP TABLE employee_copy;
-- employee 테이블에서 사번, 직원명, 급여, 연봉만 저장하는 테이블 생성
CREATE TABLE employee_copy
AS SELECT emp_id 사번, emp_name 직원명, format(salary,0) 급여, format(salary*12,0) 연봉 FROM kh.employee;

...
*/
-- 각 부서별 조회 (group별로 몇명이 있는지)
SELECT dept_code, count(*) "사원 수", sum(salary) "총 급여",
avg(coalesce(salary,0)) "평균 급여", min(salary) "최저 급여", max(salary) "최고 급여"
FROM employee
GROUP BY dept_code
-- ORDER BY dept_code;
ORDER BY 1;
...
-- 성별 별(남자/여자) 사원 수 조회
SELECT if(substr(emp_no, 8, 1) = 1, '남자', '여자') "성별", COUNT(*) "사원 수" 
FROM employee
GROUP BY 성별;

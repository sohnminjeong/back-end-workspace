...
SELECT trim(BOTH ' ' FROM '       K H       ');       -- 위의 문장과 같은 의미
...
SELECT substr('PROGRAMMING', -8, 3);   -- GRA / -는 뒤에서부터 시작을 의미
...
-- replace(emp_no, substr(remp_no, -6, 6), '******')
...
SELECT emp_name, email,
   TRIM(TRAILING '@' FROM substr(email, 1, instr(email, '@')))
...
SELECT round(123.456, 1);  -- 123.5 : 소숫점 첫번째 자리까지(두번째에서 반올림)
...
SELECT timediff('2024-01-01 00:00:00', now());   -- 129:41:17

-- 직원명, 입사일, 근무일수, 근무 개월 수, 근무 년 수  
SELECT emp_name, hire_date, datediff(now(), hire_date) "근무 일 수", 
timestampdiff(day, hire_date, now()) "근무 일 수 2",
timestampdiff(month,hire_date, now()) "근무 월 수", timestampdiff(year,hire_date, now()) "근무 년 수"
FROM employee;
...
/*
     ...
     CONVERT(값, 데이터형식[길이])
*/
SELECT CAST(1234 AS CHAR), CONVERT(123, CHAR);
...
SELECT emp_name, job_code "직급코드", format(salary,0), 
-- format(salary, 0)으로 하면 보기 편함
...
FROM employee;
-- where dept_code is not null; 가능
...
FROM employee;
-- SELECT count(*)로 하고 where ent_yn='y'도 가능 

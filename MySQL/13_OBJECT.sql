...
DROP PROCEDURE salary_calc;

-- 방금 만든 salary_calc 함수를 이용해서 연봉이 4000만원 이상인
-- 사원의 사번, 사원명, 급여, 연봉 조회 (정렬 연봉 높은 순)
SELECT emp_id, emp_name, format(salary,0) '급여', format(salary_calc(emp_id),0) '연봉' 
FROM employee
...
/*
    ...
    ex) 
    - 회원탈퇴 시 기존 회원테이블에 데이터 DELETE 후 곧바로 탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT 처리
...
CREATE TABLE tb_prodetail(
    ...
    status CHAR(2) CHECK(status IN('입고', '출고')) NOT NULL,   -- 상태
...
/*
	...
	tb_product 테이블에 매번 자동으로 재고수량 UPDATE 되게 끔 트리거 정의
